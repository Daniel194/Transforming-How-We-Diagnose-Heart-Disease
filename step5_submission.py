...
PREDICT_FILE_PATH = settings.RESULT_DIR + "prediction_calibrated_" + MODEL_NAME + ".csv"
...
def make_predict_row_step(predicted_volume):
    """
    Make prediction based on step row
    :param predicted_volume: prediction volume
...
def make_predict_row_window(predicted_volume, window_size=12):
    """
    Make prediction on row windows
    :param predicted_volume: prediction volume
...
def make_predict_row_stdev(predicted_volume, stdev):
    """
    Make prediction based on standard deviation
    :param predicted_volume: prediction volume
...
def make_predict_row_mean_stdev(predicted_volume, mean, stdev):
    """
    Make prediction based on rows mean and standard deviation
    :param predicted_volume: prediction volume
    :param mean: mean
    :param stdev: standard deviation
    :return: new prediction
...
def get_prediction_based_on_avg(age_years, sex, reference_data, field):
    ...
    :param age_years: age exprimate in years
    :param sex: sex
    :param reference_data: reference data
    :param field: field
    :return: prediction and standard deviation
    """
    ...
    while not ok:
        ...
        filtered_reference_data = filtered_reference_data[
            filtered_reference_data["age_years"] <= age_years + age_spread]
...
def generate_stdevs(from_patient1, to_patient1, from_patient2, to_patient2, window_size=30):
    """
    Create a cvs file with error, standard deviation and mean error between true values and
    predicted values
    :param from_patient1: form patent
    :param to_patient1: to patient
    :param from_patient2: from patient
    :param to_patient2: to patient
    :param window_size: windows size
    ...
    base_data = base_data[(((base_data["patient_id"] >= from_patient1) & (base_data["patient_id"] < to_patient1)) | (
        (base_data["patient_id"] >= from_patient2) & (base_data["patient_id"] < to_patient2)))]
    res = base_data[
        ["patient_id", FIELD_PREFIX + "pred_dia", STD_COL + "dia", FIELD_PREFIX + "pred_sys", STD_COL + "sys"]].copy()
    res["std_dia"] = 0
    res["std_sys"] = 0
    res["mean_dia"] = 0
    res["mean_sys"] = 0
    for dia_sys in ["dia", "sys"]:
        ...
        res = res.sort_values(by=FIELD_PREFIX + "pred_" + dia_sys)
    ...
    res.to_csv(settings.RESULT_DIR + "prediction_errors_std.csv", sep=";")
...
def make_predictions(predictions_path, from_patient, to_patient, stdevs, evaluate=True, submission_file=""):
    """
    Make prediction
    :param predictions_path: path to prediction path
    :param from_patient: start patient
    :param to_patient: end patiend
    :param stdevs: array with standard deviation
    :param evaluate: optional value
    :param submission_file: path where to be saved the submission file
    :return: nothing
    """
...
if __name__ == "__main__":
    utils.create_dir_if_not_exists(settings.RESULT_DIR + "submission_files/")
    stdevs = generate_stdevs(1, 590, 600, 700, window_size=60)
    ...
    make_predictions(PREDICT_FILE_PATH, 701, 1140, stdevs,
                     submission_file=settings.RESULT_DIR + "submission_files/submission" + MODEL_NAME + ".csv")
    print("Done , submission written to : " + settings.RESULT_DIR + "submission_files/submission" + MODEL_NAME + ".csv")
