...
class LVSegmentation(object):
    def __init__(self, checkpoint_dir='../../result/segmenter/train_result/v4/'):
        self.build()
    ...
    def build(self):
        ...
        conv_1_1 = self.conv_layer(self.x, [3, 3, 1, 16], 16, 'conv_1_1')
        conv_1_2 = self.conv_layer(conv_1_1, [3, 3, 16, 16], 16, 'conv_1_2')
        ...
        conv_3_1 = self.conv_layer(pool_2, [3, 3, 32, 64], 64, 'conv_3_1')
        conv_3_2 = self.conv_layer(conv_3_1, [3, 3, 64, 64], 64, 'conv_3_2')

        pool_3, pool_3_argmax = self.pool_layer(conv_3_2)

        conv_4_1 = self.conv_layer(pool_3, [3, 3, 64, 128], 128, 'conv_4_1')
        conv_4_2 = self.conv_layer(conv_4_1, [3, 3, 128, 128], 128, 'conv_4_2')
        conv_4_3 = self.conv_layer(conv_4_2, [3, 3, 128, 128], 128, 'conv_4_3')

        pool_4, pool_4_argmax = self.pool_layer(conv_4_3)

        conv_5_1 = self.conv_layer(pool_4, [3, 3, 128, 256], 256, 'conv_5_1')
        ...
        fc_6 = self.conv_layer(pool_5, [7, 7, 256, 2048], 2048, 'fc_6')

        deconv_fc_6 = self.deconv_layer(fc_6, [7, 7, 256, 2048], 256, 'fc6_deconv')
        ...
        deconv_5_1 = self.deconv_layer(deconv_5_2, [3, 3, 128, 256], 128, 'deconv_5_1')

        unpool_4 = self.unpool_layer2x2(deconv_5_1, pool_4_argmax)

        deconv_4_3 = self.deconv_layer(unpool_4, [3, 3, 128, 128], 128, 'deconv_4_3')
        deconv_4_2 = self.deconv_layer(deconv_4_3, [3, 3, 128, 128], 128, 'deconv_4_2')
        deconv_4_1 = self.deconv_layer(deconv_4_2, [3, 3, 64, 128], 64, 'deconv_4_1')

        unpool_3 = self.unpool_layer2x2(deconv_4_1, pool_3_argmax)

        deconv_3_2 = self.deconv_layer(unpool_3, [3, 3, 64, 64], 64, 'deconv_3_2')
        deconv_3_1 = self.deconv_layer(deconv_3_2, [3, 3, 32, 64], 32, 'deconv_3_1')

        unpool_2 = self.unpool_layer2x2(deconv_3_1, pool_2_argmax)

        deconv_2_2 = self.deconv_layer(unpool_2, [3, 3, 32, 32], 32, 'deconv_2_2')
        deconv_2_1 = self.deconv_layer(deconv_2_2, [3, 3, 16, 32], 16, 'deconv_2_1')

        unpool_1 = self.unpool_layer2x2(deconv_2_1, pool_1_argmax)

        deconv_1_2 = self.deconv_layer(unpool_1, [3, 3, 16, 16], 16, 'deconv_1_2')
        deconv_1_1 = self.deconv_layer(deconv_1_2, [3, 3, 16, 16], 16, 'deconv_1_1')

        score_1 = self.deconv_layer(deconv_1_1, [1, 1, 2, 16], 2, 'score_1')
