...
for dia_sys in ["dia" + ext, "sys" + ext]:
    ...
    print("\t".join(map(lambda x: str(x).rjust(10),
                        ["Fold", "Train-n", "Validate-n", "Raw", "Raw^2", "Raw mi", "Raw mx", "Train", "Train^2",
                         "Train mi",
                         "Train mx", "Validate", "Validate^2", "Validate mi", "Validate mx", "MAE"])))
    ...
    for fold_range in fold_ranges:
        ...
        if PREDICT_RATIO:
            print_values.append(
                round(mean_absolute_error(x_validate["pred_" + dia_sys] * y_pred / 100, real_values.fillna(0)), 2))
            new_predictions += (x_validate["pred_" + dia_sys] * y_pred / 100).map(lambda x: round(x, 2)).values.tolist()
        else:
            print_values.append(
                round(mean_absolute_error(x_validate["pred_" + dia_sys] - y_pred, real_values.fillna(0)), 2))
