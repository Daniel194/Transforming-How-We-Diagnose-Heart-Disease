...
class DeconvNet:
    def __init__(self, images, segmentations, use_cpu=False, checkpoint_dir='./checkpoints/'):
        ...
        self.saver = tf.train.Saver(tf.global_variables(), max_to_keep=5, keep_checkpoint_every_n_hours=1)  # v0.12
...
if __name__ == '__main__':
    ...
    trn_images_batch, trn_segmentations_batch = input_pipeline(args.train_record, args.batch_size, args.num_epochs)

    deconvnet = DeconvNet(trn_images_batch, trn_segmentations_batch, use_cpu=False)

    logits = deconvnet.logits

    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits, tf.cast(tf.reshape(deconvnet.y, [-1]), tf.int64), name='x_entropy')
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                if step % 1 == 0:
                    ...
                    print(format_str % (datetime.now(), step, loss_val, examples_per_sec, sec_per_batch))
